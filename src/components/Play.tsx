...
const Play: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const generateAvailableLetters = (text: string) => {
    ...
    const randomLetters = Array(randomLettersCount)
      .fill(null)
      .map(getRandomLetter);
    return [...requiredLetters, ...randomLetters].sort(
      () => Math.random() - 0.5
    );
  };
...
useEffect(() => {
  ...
  useEffect(() => {
    const getQuestions = async () => {
      if (token && msisdn) {
        setLoading(true);
        try {
          const response = await fetchGameQuestions(
            questionCount,
            msisdn,
            token
          );
          if (response?.isSuccessful && response.data.length > 0) {
            ...
            setAvailableLetters(
              generateAvailableLetters(response.data[0].text)
            );
  ...
  const submitGame = async () => {
    ...
    if (question && msisdn) {
      ...
      const response = await submitGamePlay(
        msisdn,
        question.id,
        submittedAnswer,
        hintUsed,
      ...
      <div className="max-w-sm text-center w-full">
        {loading ? (
          ...
          <>
            <div>
              <img
                className="w-max mx-auto max-sm:my-2"
                src="/icons/timer.svg"
                alt="timer icon"
              />
            ...
            <div className="my-10">
              ...
              <div className="flex flex-wrap justify-center item-center gap-3">
                {question?.text.split("").map((letter, index) => (
                  <p
                    key={index}
                    className="p-2.5 shadow-dark bg-teal rounded text-center capitalize text-white font-bold"
                  >
                    {letter}
                  </p>
                ))}
            ...
            <div className="bg-[#002B2DB2] p-6 rounded-[20px]">
              ...
              <div className="flex justify-center capitalize items-center gap-1 my-4">
                {selectedLetters.map((letter, index) => (
                  <div
                    key={index}
                    className={`p-2 py-2.5 lg:text-lg text-black font-bold bg-white cursor-pointer shadow-dark h-12 w-10 rounded text-center ${
                      letter ? "border-[#00F0FF] border-2" : ""
                    }`}
                    onClick={() => handleLetterDeselect(index)}
                  >
                    {letter}
                  </div>
                ))}
              ...
              <div className="flex flex-wrap justify-around gap-2 mt-6 ">
                {availableLetters.map((letter, index) => (
                  <button
                    key={index}
                    className={`p-2.5 shadow-dark  capitalize rounded text-center text-white font-bold ${
...
//         setLoading(true);
//         try {
//             await onPlayAgain(); // Assuming onPlayAgain is an async function
//         } catch (error) {
//             console.error('Error in playing again:', error);
//         }
//         setLoading(false);
//     };
...
//                     <FFButton 
//                         className="md:w-5/6 flex items-center justify-center" 
