...
const Login: React.FC = () => {
  ...
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const allowedCountries: CustomCountryCode[] = [
    "NG",
    "CM",
    "GH",
    "CI",
  ...
  const handleLogin = async () => {
    ...
    const countryAlpha2Code = getCountryAlpha2Code(
      cleanMsisdn,
      allowedCountries
    );
    if (!countryAlpha2Code) {
    ...
    try {
      // const subscriptionResponse = await checkSubscriptionStatus(cleanMsisdn);

      //   if (!subscriptionResponse.isSuccessful || !subscriptionResponse.data?.isActive) {

      //     console.error('Subscription check error:', subscriptionResponse.message);
      //     setIsSubscribed(false);
      //   setError('You are not subscribed to Fastest Finger. Please subscribe to continue.');
      ...
      setError(
        (error as Error).message ||
          "Login failed: Please check your credentials or subscribe."
      );
      console.error("Login error:", error);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white font-inria text-ffgray mx-6 my-10">
      <div className="bg-[#f9f9f9] py-16 px-8 rounded shadow-custom w-full md:w-[610px] text-center">
        <img src="/images/FFlogo.svg" alt="Logo" className="mx-auto h-8" />
        <h2 className="font-semibold my-2 italic">
        ...
        <FFButton
          ...
          className="w-full md:w-3/5 mx-auto mt-20"
          onClick={handleLogin}
      ...
      <div className="my-8 text-center hidden">
        <p className="bg-[#009A4933] py-4 px-24 italic shadow-custom">
